...
const PlanCard = ({ plan, onEdit, onDelete, onToggleStatus }) => {
  const occupancyRate = Math.round((plan?.enrolled / plan?.capacity) * 100);
    ...
    <div className="bg-card border border-border rounded-xl p-6 transition-smooth hover:shadow-lg">
      ...
      <div className="mb-4">
        ...
        <div className="flex flex-wrap gap-2">
          {plan?.schedule?.map((sch, idx) => (
            <span
              key={idx}
              className="px-2 py-1 bg-muted/50 border border-border/50 rounded text-xs text-foreground"
            >
              {sch?.day} {sch?.time}
            </span>
          ))}
      ...
      <div className="flex gap-2 pt-4 border-t border-border">
        ...
        <Button
          variant="outline"
          size="sm"
          iconName={plan?.status === 'active' ? 'XCircle' : 'CheckCircle'}
