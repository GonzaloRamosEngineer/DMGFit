...
const normalizeSchedule = (schedule = []) => {
  return (schedule || [])
    .map((slot) => {
      ...
      const remaining = Math.max(0, Number(slot.remaining ?? capacity));
...
const PlanAvailabilityGridModal = ({ plan, onClose }) => {
  ...
  const columnsByDay = useMemo(() => {
    return DAYS.map((day, dayIndex) => {
      const slots = normalizedSchedule.filter(
      ...
      return {
        ...
        dayIndex,
        slots,
        grouped: groupSlotsByPeriod(slots),
      };
        ...
        <div className="overflow-y-auto custom-scrollbar flex-1 bg-slate-50">
          {columnsByDay.length === 0 ? (
            ...
            <div className="p-4 sm:p-6">
              {/* Desktop/table layout with horizontal scroll if many days */}
                  ...
                  {columnsByDay.map((group) => (
                    <div key={group.dayIndex} className="flex flex-col">
                      ...
                      <div className="mt-3 space-y-4">
                        {Object.entries(group.grouped).map(([periodKey, slots]) => {
