...
const AutomatedReminderControl = ({ settings, onSettingsChange, loading = false }) => {
  ...
  const Toggle = ({ label, checked, onChange }) => (
    <div className="flex items-center justify-between py-2 cursor-pointer" onClick={() => onChange(!checked)}>
      ...
      <div className={`w-10 h-5 flex items-center rounded-full p-1 duration-300 ease-in-out ${checked ? 'bg-success' : 'bg-slate-600'}`}>
  ...
  const ChannelCheckbox = ({ label, value, checked, onChange }) => (
    <label className={`flex items-center gap-2 p-2 rounded-lg border cursor-pointer transition-all ${
      checked ? 'bg-primary/20 border-primary/50 text-primary' : 'bg-slate-800 border-slate-700 text-slate-400 hover:border-slate-600'
    }`}>
      <input 
        type="checkbox" 
        className="hidden" 
    ...
    <div className="bg-slate-800/50 rounded-2xl border border-slate-700/50 overflow-hidden transition-all duration-300">
      ...
      >
        <div className="flex items-center gap-3">
          <div className={`p-2 rounded-lg ${settings?.enabled ? 'bg-success/20 text-success' : 'bg-slate-700 text-slate-400'}`}>
            ...
            <>
              ...
              <div>
                ...
                <select 
                  ...
                  className="w-full bg-slate-900 border border-slate-700 rounded-xl px-3 py-2 text-sm text-white outline-none focus:border-primary"
                >
              ...
              <div className="flex gap-2 pt-2">
                <button className="flex-1 py-2 bg-primary hover:bg-primary/90 text-primary-foreground rounded-lg text-xs font-bold uppercase tracking-wider transition-colors shadow-md">
                  Guardar
                </button>
