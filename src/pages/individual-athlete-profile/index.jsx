...
const extractPlanSlotsRows = (response) => {
...
const normalizeAvailablePlanSlots = (response) => {
  const rows = extractPlanSlotsRows(response);

  return rows
    .map((row) => {
      const schedule =
        normalizeRelation(row.weekly_schedule) ||
        normalizeRelation(row.weeklySchedule) ||
        row;

      const weeklyScheduleId =
        row.weekly_schedule_id ||
        row.weeklyScheduleId ||
        schedule?.id ||
        row?.id ||
        null;

      const capacity = Number(schedule?.capacity ?? row?.capacity ?? 0);
      ...
      return {
        ...
        day_of_week: Number(schedule?.day_of_week ?? row?.day_of_week ?? 0),
        start_time: schedule?.start_time ?? row?.start_time ?? null,
        end_time: schedule?.end_time ?? row?.end_time ?? null,
        capacity,
        remaining,
      };
    })
...
const IndividualAthleteProfile = () => {
  ...
  const canModifySchedule =
    canManageMembership &&
    Number(profileData.athlete?.visits_per_week || 0) > 0 &&
    !!profileData.athlete?.plan_id;

  const handleOpenScheduleModal = async () => {
    if (!canModifySchedule || !profileData.athlete?.plan_id) return;

    setLoadingScheduleModal(true);
    try {
      const response = await fetchPlanSlots(profileData.athlete.plan_id);
      const normalizedSlots = normalizeAvailablePlanSlots(response);
  ...
  return (
    <>
      <Helmet>
        <title>
          {profileData.athlete
            ? `${profileData.athlete.name} - Perfil`
            : "Cargando..."}{" "}
          - VC Fit
        </title>
      </Helmet>
