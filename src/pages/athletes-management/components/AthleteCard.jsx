...
const AthleteCard = ({
  ...
  if (loading && layout === "table") {
    return (
      <>
        {/* Skeleton tabla (contenedor ancho) */}
        <div className={`hidden @2xl:grid ${gridLayout} gap-3 px-5 py-3 items-center`}>
        ...
        {/* Skeleton tarjeta (contenedor angosto) */}
        <div className="@2xl:hidden p-4 space-y-3">
          <div className="flex gap-3 items-center">
            <Skeleton className="w-9 h-9 rounded-xl" />
            <div className="space-y-2 flex-1"><Skeleton className="h-3 w-1/2" /><Skeleton className="h-2 w-2/3" /></div>
            <Skeleton className="h-5 w-16 rounded-lg" />
          </div>
          <Skeleton className="h-2 w-1/2" />
          <div className="flex gap-3"><Skeleton className="h-5 w-16 rounded-lg" /><Skeleton className="h-5 w-16 rounded-lg" /></div>
        </div>
      </>
    );
  }
  ...
  const goToProfile = () => navigate(`/individual-athlete-profile/${athlete.id}`);
  const scoreColor = athlete.performanceScore >= 80
    ? 'text-success'
    : athlete.performanceScore < 50 ? 'text-error' : 'text-text-secondary';

  // Avatar con indicador de estado (compartido entre vista tabla y tarjeta).
  const avatar = (
  ...
  // Acciones (registrar pago + ver perfil). Compartidas entre ambas vistas.
  ...
  if (layout === "table") {
    return (
      <>
        ...
        <div className={`hidden @2xl:grid ${gridLayout} gap-3 px-5 py-3 items-center transition-colors group hover:bg-muted/60`}>
          {/* 1. Atleta Info */}
          <div className="flex items-center gap-2.5 min-w-0 cursor-pointer" onClick={goToProfile}>
            {avatar}
          ...
          <div className="flex items-center gap-4 min-w-0">
            ...
            <div className="flex flex-col">
              ...
              <span className={`text-xs font-black ${scoreColor}`}>{athlete.performanceScore}</span>
            </div>
          </div>
        ...
        <div className="@2xl:hidden p-4 flex items-start gap-3 transition-colors group hover:bg-muted/60">
          {/* Avatar */}
          <div className="cursor-pointer" onClick={goToProfile}>{avatar}</div>
          ...
          <div className="min-w-0 flex-1 cursor-pointer" onClick={goToProfile}>
            ...
            <p className="mt-1.5 text-xs font-bold text-text-secondary truncate">
              ...
              <span className="text-text-tertiary"> · ${athlete.planPrice}</span>
            </p>
          </div>

          {/* Columna derecha: estado arriba, acciones abajo */}
