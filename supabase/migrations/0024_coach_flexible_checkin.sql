-- 0024_coach_flexible_checkin.sql
-- Asistencia FLEXIBLE del profesor en el kiosco (decisión de producto 2026-07-18).
-- Antes: la rama de profes de kiosk_check_in RECHAZABA si el profe no tenía un
-- turno asignado a esa hora (COACH_OUT_OF_WINDOW) o si había más de uno (AMBIGUOUS_SLOT).
-- Ahora: el profe ficha SIEMPRE (registra "cuándo vino"); si hay un turno que
-- matchea se adjunta como dato (weekly_schedule_id), pero no es requisito.
-- Dedupe: un fichaje por coach por día (ALREADY_TODAY si repite).
--
-- El cuerpo se tomó VERBATIM de la definición viva; SOLO se reescribió la rama
-- 'coach/profesor'. La rama de ATLETAS queda byte-idéntica (verificado por el
-- generador). Generado por scratchpad_gen_0024.mjs.
