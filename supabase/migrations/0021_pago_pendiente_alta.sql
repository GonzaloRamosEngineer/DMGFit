-- 0021_pago_pendiente_alta.sql
-- "Cobrar ahora / cobrar despues" en el alta de atletas.
--  * create_full_athlete_atomic: register_payment=false -> crea el pago como 'pending'
--    (deuda registrada) en vez de 'paid'. El contador de accesos se crea igual (ya lo hacia).
--  * kiosk_check_in: un atleta SIN pago 'paid' pasa con WARNING "cuota pendiente"
--    (permitido + descuenta acceso), en vez de verde limpio. Cierra el hueco de los
--    socios sin pago registrado.
--  * Nuevo reason_code OK_PENDING (FK de access_logs.reason_code).

insert into public.kiosk_reason_codes (code, category, description, is_active)
values ('OK_PENDING','warning','Acceso permitido con cuota pendiente',true)
