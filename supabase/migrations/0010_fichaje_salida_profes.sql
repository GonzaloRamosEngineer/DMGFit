-- 0010_fichaje_salida_profes.sql
-- Fichaje de ENTRADA y SALIDA para profesores.
-- Antes: el profe fichaba 1 vez/día (entrada) y el 2º fichaje decía "ya registraste".
-- Ahora: el 1º fichaje del día es la ENTRADA y el 2º es la SALIDA (se guarda en la
-- misma fila, columna check_out_time). El 3º+ dice "ya registraste entrada y salida".
--
-- El kiosco muestra "Entrada registrada" / "Salida registrada" SIN horas (el profe no
-- las ve). Las horas trabajadas se calculan (check_out - check_in) y se muestran solo
-- en la sección de administración "Asistencia de Profesores" (/coach-attendance).
--
-- Solo cambia la RAMA COACH de kiosk_check_in; la rama atleta queda igual que 0006.
-- Reason_code: en la SALIDA se actualiza solo check_out_time (la fila conserva
-- reason_code='OK', registrado). El código 'OK_CHECKOUT' viaja solo en el jsonb de
-- respuesta (no se persiste), así que no toca kiosk_reason_codes.

-- (1) Columna de salida (idempotente)
ALTER TABLE "public"."access_logs" ADD COLUMN IF NOT EXISTS "check_out_time" timestamp with time zone;

-- (2) kiosk_check_in con entrada/salida en la rama coach
...
begin
  ...
  if v_role = 'coach' or v_role = 'profesor' then
    ...
    -- 1er fichaje del día = ENTRADA; 2º = SALIDA (misma fila). Lock por coach+día.
    ...
    -- Buscar el fichaje de hoy (la entrada) y si ya tiene salida.
    ...
    if v_log_id is not null and v_checkout_at is not null then
      -- Ya registró entrada y salida hoy.
      v_reason_code := 'ALREADY_TODAY';
      v_message := 'Ya registraste entrada y salida hoy. ¡Buen descanso!';
      v_details := jsonb_build_object(
        'remaining', null, 'period_start', null, 'period_end', null, 'expires_at', null,
        'days_late', null, 'grace_days', p_grace_days, 'plan_name', null,
        'actor_type', v_actor_type, 'weekly_schedule_id', v_weekly_schedule_id,
        'local_date', v_local_date, 'already_today', true, 'phase', 'done'
      );
      return jsonb_build_object(
    ...
    elsif v_log_id is not null then
      -- SALIDA: 2º fichaje del día -> registrar la salida en la misma fila.
