-- 0014_asistencia_profes_multi_registro.sql
-- Pedido de Cris (2026-08-06), vía Javier: un profe fichó su entrada a la mañana,
-- fichó su salida al mediodía, y cuando volvió a la tarde a fichar de nuevo el
-- kiosco le dijo "ya registraste entrada y salida hoy" y no lo dejó. Con 0010 el
-- día de un profe era UNA sola fila (1er fichaje = entrada, 2º = salida, 3º en
-- adelante rechazado con ALREADY_TODAY).
--
-- Ahora: el profe puede fichar varios ciclos de entrada/salida en el mismo día
-- (cada ciclo es una fila distinta de access_logs). La regla pasa a ser sobre el
-- ÚLTIMO fichaje del día: si está abierto (sin salida), el próximo fichaje lo
-- cierra (salida); si está cerrado (con salida) o no hay ninguno todavía, el
-- próximo fichaje abre un ciclo nuevo (entrada). Ya no hay techo de fichajes/día.
--
-- Además, charlando la corrección manual (0013, que sólo corrige filas que ya
-- existen), a Cris le gustó poder ELLA registrar directamente una presencia de un
-- profesor sin que él haya fichado nada (p.ej. si el profe avisa por WhatsApp que
-- se olvidó el DNI). Eso es lo que 0013 dejaba "para decidir aparte" — se agrega
-- ahora como `admin_create_coach_attendance`, reusando las mismas columnas
-- *_edited_at/attendance_edited_by para marcarlo "a mano" en la UI.

-- (1) kiosk_check_in: la rama coach pasa a permitir N ciclos por día.
--     El idempotency_key de coach ya no es "un valor por día" (coach+día), porque
--     ahora puede haber varias filas insertadas el mismo día: se le agrega el
--     número de ciclo dentro del día para seguir siendo único
--     (uq_access_logs_idempotency_coach sigue vigente, ver 0000_baseline).
...
begin
  ...
  if v_role = 'coach' or v_role = 'profesor' then
    ...
    -- El profe puede fichar varios ciclos entrada/salida por día. Lock por
    -- coach+día para serializar fichajes simultáneos del mismo profe.
    perform pg_advisory_xact_lock(hashtextextended(v_coach_id::text || ':day:' || v_local_date::text, 0));

    -- Último ciclo del día (si hay). Si está abierto (sin salida), este fichaje lo
    -- cierra. Si está cerrado, o no hay ninguno, este fichaje abre un ciclo nuevo.
    select al.id, al.check_out_time
    into v_log_id, v_checkout_at
    ...
    -- Cuántos ciclos ya tiene hoy este profe (para el próximo número de ciclo).
    select count(*) + 1
    into v_cycle_number
    from public.access_logs al
    where al.coach_id = v_coach_id
    ...
    if v_log_id is not null and v_checkout_at is null then
      -- SALIDA: cierra el ciclo abierto (misma fila).
      update public.access_logs set check_out_time = p_now where id = v_log_id;
      v_reason_code := 'OK_CHECKOUT';
      v_message := '¡Chau, ' || coalesce(v_full_name, 'profe') || '! Salida registrada.';
      v_details := jsonb_build_object(
        'remaining', null, 'period_start', null, 'period_end', null, 'expires_at', null,
        'days_late', null, 'grace_days', p_grace_days, 'plan_name', null,
        'actor_type', v_actor_type, 'weekly_schedule_id', v_weekly_schedule_id,
        'local_date', v_local_date, 'phase', 'out'
      );
      return jsonb_build_object(
    ...
    -- ENTRADA: abre un ciclo nuevo (1er fichaje del día, o el profe volvió tras
    -- haber cerrado un ciclo anterior).
    v_reason_code := 'OK';
    v_message := '¡Hola, ' || coalesce(v_full_name, 'profe') || '! Entrada registrada.';
    v_idempotency_key := v_coach_id::text || ':day:' || v_local_date::text || ':' || v_cycle_number::text;
    ...
    v_details := jsonb_build_object(
      'remaining', null, 'period_start', null, 'period_end', null, 'expires_at', null,
      'days_late', null, 'grace_days', p_grace_days, 'plan_name', null,
      'actor_type', v_actor_type, 'weekly_schedule_id', v_weekly_schedule_id, 'local_date', v_local_date, 'phase', 'in'
    );
...
-- (2) admin_create_coach_attendance: crear una presencia de profesor DESDE CERO,
--     cargada por admin (sin que el profe haya fichado nada en el kiosco). Es lo
--     que 0013 dejó explícitamente "para decidir aparte" si hacía falta.
--     Se marca "a mano" con las mismas columnas de 0013 (attendance_edited_by,
--     check_in_edited_at, check_out_edited_at) para que la UI la distinga igual
--     que una corrección: el badge "A mano" no tiene que saber si el registro es
--     nuevo o corregido, sólo si ESE campo lo cargó administración.
CREATE OR REPLACE FUNCTION "public"."admin_create_coach_attendance"(
  "p_coach_id" "uuid",
  "p_date" "date",
  "p_check_in" "text",
...
begin
  if not public.is_admin(auth.uid()) then
    raise exception 'FORBIDDEN: solo admin puede registrar asistencia a mano';
  end if;

  if p_coach_id is null then
    raise exception 'Falta el profesor.';
  end if;

  if not exists (select 1 from public.coaches where id = p_coach_id) then
    raise exception 'No se encontró el profesor.';
  end if;
  ...
  v_in := (p_date + v_in_t) at time zone v_tz;
  ...
  if p_check_out is not null and trim(p_check_out) <> '' then
    ...
    v_out := (p_date + v_out_t) at time zone v_tz;
  ...
  -- Mismo lock e idempotency_key que kiosk_check_in, para que un fichaje real del
  -- kiosco y una carga manual de admin no puedan pisarse el número de ciclo.
  perform pg_advisory_xact_lock(hashtextextended(p_coach_id::text || ':day:' || p_date::text, 0));

  select count(*) + 1
  into v_cycle_number
  from public.access_logs al
  where al.coach_id = p_coach_id
    and al.local_checkin_date = p_date
    and al.access_granted is true;

  v_idempotency_key := p_coach_id::text || ':day:' || p_date::text || ':' || v_cycle_number::text;

  insert into public.access_logs (
    athlete_id, coach_id, access_granted, reason_code, local_checkin_date, idempotency_key,
    check_in_time, check_out_time, attendance_edited_by, check_in_edited_at, check_out_edited_at
  ) values (
    null, p_coach_id, true, 'OK', p_date, v_idempotency_key,
    v_in, v_out, auth.uid(), timezone('utc', now()), case when v_out is not null then timezone('utc', now()) else null end
  )
...
ALTER FUNCTION "public"."admin_create_coach_attendance"("uuid", "date", "text", "text") OWNER TO "postgres";
GRANT EXECUTE ON FUNCTION "public"."admin_create_coach_attendance"("uuid", "date", "text", "text") TO "authenticated", "service_role";
