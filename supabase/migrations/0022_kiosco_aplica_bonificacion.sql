-- 0022_kiosco_aplica_bonificacion.sql
-- El kiosco también tiene que aplicar la bonificación del atleta.
--
-- QUÉ ARREGLA
--   Hay tres lugares que generan la cuota mensual: el cron nocturno, el botón
--   "Generar Periodo" y el bloque 9b de `kiosk_check_in` (cuando el atleta ficha y le
--   toca cuota nueva). La 0017 enseñó a los dos primeros a aplicar `discount_percent`,
--   pero el del kiosco quedó afuera.
--
--   Consecuencia: si un bonificado fichaba antes de que corriera el cron, su cuota salía
--   al precio de lista completo. Pascual Gordillo (50%) hubiera recibido $70.000 en lugar
--   de $35.000, y Cris habría tenido que corregirlo a mano — justo lo que 0017 vino a
--   evitar. Nadie se enteraba: la cuota se ve bien formada, sólo está mal el monto.
--
--   Detectado al parchear la 0021. Se dejó fuera de ese hotfix a propósito: en una
--   urgencia, un solo cambio por vez.
--
-- QUÉ SE CAMBIA
--   Sólo el INSERT del bloque 9b: usa `athlete_effective_amount()` para el monto y deja
--   cargado el descuento (`percent` + el %), igual que los otros dos generadores, para que
--   el panel y el comprobante lo muestren parejo. `base_amount` sigue siendo el precio de
--   lista. El resto del cuerpo es exactamente el de producción, traído con
--   pg_get_functiondef.
...
begin
  ...
  select v_athlete_id,
         -- Lo que realmente paga: precio de lista menos su bonificación (0017).
         public.athlete_effective_amount(
           coalesce(nullif(a.plan_tier_price, 0), pl.price, 0),
           coalesce(a.discount_percent, 0)),
         coalesce(nullif(a.plan_tier_price, 0), pl.price, 0),
         ...
         case when coalesce(a.discount_percent, 0) > 0 then 'percent' else null end,
         case when coalesce(a.discount_percent, 0) > 0 then a.discount_percent else 0 end
  from public.athletes a
  left join public.plans pl on pl.id = a.plan_id
  where a.id = v_athlete_id
    and not exists (
