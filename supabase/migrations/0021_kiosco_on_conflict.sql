-- 0021_kiosco_on_conflict.sql
-- HOTFIX: el kiosco quedó rechazando a TODOS los atletas.
--
-- QUÉ PASÓ
--   La migración 0018 cambió el índice `payments_athlete_period_uidx` para que ignore las
--   cuotas anuladas (le agregó `and status <> 'void'`). Se actualizaron los dos
--   generadores de cuotas para que repitieran ese predicado en su ON CONFLICT... pero
--   `kiosk_check_in` también inserta una cuota (bloque 9b, migración 0006) con el
--   predicado viejo, y esa no se tocó.
--
--   Postgres necesita que el predicado del ON CONFLICT implique el del índice parcial para
--   poder inferirlo como árbitro. Al no coincidir, el INSERT falla con:
--       "there is no unique or exclusion constraint matching the ON CONFLICT specification"
--   y como el kiosco atrapa cualquier error como denegación, el cartel que veían los
--   atletas era ACCESO DENEGADO con ese texto.
--
--   Reportado por Cris el 2026-09-23 19:29 ("no se están pudiendo registrar", turno tarde).
--   Estuvo roto desde que se aplicó 0018 ese mismo día.
--
-- QUÉ SE CAMBIA
--   Una sola línea: el predicado del ON CONFLICT del bloque 9b. El cuerpo de la función es
--   exactamente el que estaba en producción, traído con pg_get_functiondef para no
--   reescribir nada de memoria.
--
-- LECCIÓN (ya estaba documentada y se volvió a pisar): al tocar un índice parcial hay que
--   buscar TODOS los ON CONFLICT que lo usan, no sólo los del archivo que se está editando:
--     select proname from pg_proc where prosrc like '%on conflict (athlete_id, period)%';
